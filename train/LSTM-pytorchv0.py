...
test_profiles = [2, 3, 4]  # 专门用于测试的profile

time_steps = 20

# 4. 数据预处理函数
def create_sequences(group, time_steps=20):
...
def prepare_datasets(data, exclude=[], test_only=[]):
    X_train, y_train = np.empty((0, time_steps, len(features))), np.empty((0, len(targets)))
    X_test, y_test = np.empty((0, time_steps, len(features))), np.empty((0, len(targets)))

    for profile_id, group in data.groupby('profile_id'):
        X_seq, y_seq = create_sequences(group, time_steps)
        if len(X_seq) == 0:
            continue
